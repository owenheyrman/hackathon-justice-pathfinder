...
from haystackPipeline import HaystackPipeline


config = load_config()
#client = AsyncOpenAI(api_key=config["openAI_key"])
async def query_chatgpt(prompt, model="gpt-4", api_key=config["openAI_key"]):
    """
    Queries ChatGPT-4 with a given prompt.

    Args:
        prompt (str): The prompt to send to ChatGPT-4.
...
async def main():
    haystackPipeline = HaystackPipeline()
    ...
    if user_input:
        ...
        if user_input:
            ...
            #llm_answer = llm_chain.run(user_input = user_input, 
                                       #chat_history=load_last_k_text_messages(get_session_key(), config["chat_config"]["chat_memory_length"]))
            history_messages = load_last_k_text_messages(get_session_key(), 5)
            history_string = ' '.join(map(str, history_messages))
            #llm_answer = await query_chatgpt("history: " + history_string + " Human: " + user_input + " AI: ")
            llm_answer = haystackPipeline.run(user_input)
            save_text_message(get_session_key(), "human", user_input)
            save_text_message(get_session_key(), "ai", llm_answer)
            user_input = None
